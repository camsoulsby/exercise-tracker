...
export const getTotalRepsForPeriod = async (
  userId: string,
  disciplineId: string,
  startDate: Date,
  endDate: Date
  ...
  const filteredData = mappedData.filter(
    (set) =>
      set.timeStamp.toDate() >= startDate && set.timeStamp.toDate() <= endDate
  );
...
export const getMostRecentSetDate = async (
  userId: string,
  disciplineId: string
) => {
  const userDoc = doc(db, "users", userId);
  ...
  const lastSetDate = new Date(
    Math.max(...mappedData.map((set) => set.timeStamp.getTime()))
  );
  return lastSetDate;
};
...
export const addSet = async (
  userId: string,
  disciplineId: string,
  set: { timeStamp: Date; reps: number }
) => {
  const userDoc = doc(db, "users", userId);
  const disciplinesRef = collection(userDoc, "disciplines");
  const disciplineDoc = doc(disciplinesRef, disciplineId);
